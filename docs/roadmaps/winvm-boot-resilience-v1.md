...
| WBR-005 | Resolução correta em cada tela | Evidência 2026-08-20: boot direto docked (`card1-DP-1` conectado) escolheu eDP-1 1280×800 porque `windows-vm-session.sh` força `PZ_DISPLAY_PRIMARY_TARGET=internal`; e a VM nova (perfil `compat`/QXL, sem driver QXL no convidado) rodou 1024×768 em janela minúscula com bordas (foto do usuário). Correção: default do session vira `auto` (display-session já resolve docked → modo físico do monitor por EDID; handheld → 1280×800; override `PZ_WINDOWS_VM_PRIMARY_DISPLAY=internal` preservado) e o boot direto prefere `virtio-gl` (honra `xres/yres` do env; driver viogpudo já instalado pelo provisionador) com fallback `compat` | Fixture `PZ_DISPLAY_SYSFS_ROOT` com DP-1 conectado+modes: session resolve conector externo com `auto`; handheld continua 1280×800 | validação física Deck LCD + monitor pendente | in_progress |
