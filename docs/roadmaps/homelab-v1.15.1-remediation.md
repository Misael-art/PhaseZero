...
| Remediação REV-001/002/003/018 | `codex/rev-remediation` (sobre `9d6be9d`): maturidade antes de Docker no `up` (rc 69 honesto sem daemon), volume registrado antes da primeira mutação no restore + `recoveryRequired` verdadeiro, `cmd_down` antes do snapshot pré-restore, sqlite `.backup` na origem viva com reason por volume | `tests/linux-homelab.sh` exit 0 com daemon presente E com docker indisponível; pytest player 40; contratos agent/web 38; `git diff --check`; ShellCheck sem achado novo vs baseline | 2026-09-07 |
