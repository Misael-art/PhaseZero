...
| Hermes/Odysseus | diagnóstico conjunto read-only publicado na branch; `assistant-private` expõe cobertura Compose incompleta e `up --profile` falha fechado; Hermes/Odysseus seguem `ready:false` sem proveniência confiável, Podman/Tailscale/Ollama e release gate | `pz ai workspaces doctor|plan`, suíte hermética, auditoria oficial GitHub | 2026-08-22 |
| Host pós-diagnóstico IA | pacote `1.16.6-1`; único container `ai-memory` saudável preservado; 9Router saudável em `127.0.0.1:20128`; portas 7000/11434 livres; 45 mounts, 0 QEMU, `grubenv` mtime `1785849543`; nenhum workload/serviço instalado ou iniciado | `pacman -Q`, `docker ps`, `ss`, `systemctl --user`, `findmnt`, `ps`, `stat` | 2026-08-22 |
...
| 2026-08-22 | Codex | `codex/hermes-odysseus-diagnostic` (`/mnt/sdcard/Projects/pz-hermes-odysseus-diagnostic`) | Fases 4/5/6: verdade de perfil, diagnóstico profundo e UX segura Hermes/Odysseus | `2d5386f`, `3d5294e` | pytest 576/576; runner 43 verdes + `linux-guest-recovery` reproduzido verde em storage persistente; `linux-ai`, `linux-homelab`, `linux-agent-workspaces`, ShellCheck, bash-n e diff-check verdes | Diagnóstico/planos redigidos disponíveis; instalação/configuração bloqueia antes da primeira escrita sem release gate; Odysseus exige allowlist commit+tree e não duplica chave 9Router. Upstream Hermes sem asset/checksum imutável; Odysseus sem release/tag. Host preservado. **Próximo**: revisão/CI; criar allowlists somente quando upstream publicar proveniência verificável; manter workloads desligados. |
