...
| HL-WIN-DISPLAY-001 | Boot GRUB WinVM usa tela e resolução esperadas em Steam Deck portátil/dock | helper casa EDID conectado com `kwinoutputconfig.json`; Gamescope recebe conector/modo explícitos; QEMU anuncia `xres`/`yres` e GTK usa `zoom-to-fit` + fullscreen | `tests/linux-windows-vm.sh`; `tests/linux-windows-vm-graphics.sh`; `tests/linux-steamos-ux.sh`; prova host resolve `DP-1` 2560x1080 e runtime instalado casa com `3302a3b` | branch `codex/winvm-display-contract`; CI remota pendente | in_progress | Validação física final exige próximo boot GRUB; fallback sem Gamescope continua usando KWin |
...
| Boot direto Windows — tela | pacote host segue `1.16.3-1`; runtime do commit `3302a3b` instalado com backup `20260812-234252.q9kZ40`; dock resolve `DP-1` em 2560x1080 via Gamescope | `windows-vm-session --validate`, `boot runtime-check`, hashes dos artefatos | 2026-08-12 |
...
| 2026-08-12 | Codex | `codex/winvm-display-contract` (`/mnt/sdcard/Projects/pz-winvm-display`) | HL-WIN-DISPLAY-001: saída, resolução e fullscreen no boot GRUB | `3302a3b`; branch enviada ao `origin` | WinVM completa exit 0; gráficos exit 0; SteamOS UX exit 0; Bash/ShellCheck; `git diff --check`; prova host `DP-1` 2560x1080 | Runtime/GRUB atualizado com backup `20260812-234252.q9kZ40`; pacote permanece `1.16.3-1`; próximo boot físico confirma apresentação final. |
