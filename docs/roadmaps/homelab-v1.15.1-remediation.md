...
| HL-IMG-001 | Jornada leiga de imagem: achar ISO, ler edições por índice, reproduzir, habilitar no boot, restaurar GRUB e remover com segurança | `image_registry.py` (registro versionado `schemaVersion 1`, escrita atômica 0600, idempotente por `sha256`, fail-closed em versão futura); `media scan --json` aditivo em `media-inspect.sh` (bases canônicas de `detect_windows_iso`, `maxdepth 4`); `ImageManagerDialog` reutiliza `media inspect`, `ProvisionPlayerWindow.open`, `windows.boot.install` e `boot.safe-menu`; action `windows.images.manage` (não-elevada, não-mutável) interceptada no `main_window` | `tests/test_image_registry.py` (13); `tests/test_image_manager_dialog.py` (14, incl. regressões de `moveToTrash` tupla e lote que não aborta); `tests/test_windows_vm_ui.py` (botão emite a action); `tests/linux-windows-vm.sh` (scan dedupe + schema de `inspect` inalterado) | (preencher com run do PR/main após CI) | in_progress | Remoção de arquivo só via `QFile.moveToTrash` (reversível) e sempre com consentimento; boot/GRUB continuam passando pelo fluxo elevado com preview, nunca com `--yes` automático |
...
| Gestão de imagens WinVM | branch `codex/winvm-image-manager` (worktree `/tmp/pz-winvm-image-manager`) a partir de `0abc5ad`; registro versionado, `media scan`, `ImageManagerDialog` e action `windows.images.manage` | pytest 535 + 9; `tests/linux-windows-vm.sh` exit 0; `tests/test_provision.sh` 248/0; ShellCheck 0.11 nos arquivos alterados; `bash -n` global; `git diff --check` | 2026-08-11 |
...
| 2026-08-11 | Codex + Claude | `codex/winvm-image-manager` (`/tmp/pz-winvm-image-manager`) | HL-IMG-001 Fases A–E: encerramento de temas, registro de imagens, `media scan`, `ImageManagerDialog`, wiring e testes | `abfc5c5` (docs temas), `d63b66a` (registro), `3a4da5f` (scan), `1b2fad5` (dialog), `117d430` (wiring), `03ae621` (correções de revisão) | pytest 535 + 9; `tests/linux-windows-vm.sh` exit 0; `tests/test_provision.sh` 248/0; `tests/runner.sh`; ShellCheck 0.11; `bash -n` global; `git diff --check` | Revisão encontrou e corrigiu dois defeitos reais no dialog (`QFile.moveToTrash` estático devolve tupla sempre verdadeira, e falha de `inspect` abortava o resto do lote), ambos cobertos por testes que falham contra a implementação anterior. Nenhuma mutação de GRUB/`grubenv`/VM/discos/mounts. **Próximo**: push, PR para `main`, CI verde e release `v1.16.2` |
