...
| AISR-010 | Host roda cópia divergente de `claude_code_manager.py` em `~/.local/share/cc-installer/tools/phasezero-claude-manager/` (2305 linhas, 2026-08-22, ausente de qualquer commit; repo tem 1878): origem do preflight de login `upstream-auth-disabled` e do runner que fixa 0.4.19. Decidir: portar para o repo com testes, ou reinstalar a versão do repo | `diff` entre as duas cópias; `git log --all -S upstream-auth-disabled` vazio | pending (decisão do operador) |
