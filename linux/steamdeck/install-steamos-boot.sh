...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        --target-root=*)
            ...
            ;;
        --target-user)
            [ -n "${2:-}" ] || { pz_error "--target-user requires a user"; exit 1; }
...
steam_plus_fallback_content() {
    # Resolve the gamescope-session-plus output vars at install time so the Game
    # Mode session boots at the docked monitor's native resolution (or the Deck
    # LCD 1280x800 when handheld). The drop-in is sourced with `set -a` by the
    # ChimeraOS gamescope-session-plus launcher.
    local session_vars w h conn
    session_vars="$(pz_display_resolved_session_vars 2>/dev/null || true)"
    if [ -n "$session_vars" ]; then
        w="$(printf '%s\n' "$session_vars" | sed -n '1p')"
        h="$(printf '%s\n' "$session_vars" | sed -n '2p')"
        conn="$(printf '%s\n' "$session_vars" | sed -n '3p')"
    else
        w=1280; h=800; conn='*,eDP-1'
    fi
    local width_q height_q connector_q
    printf -v width_q '%q' "$w"
    printf -v height_q '%q' "$h"
    printf -v connector_q '%q' "$conn"
    cat <<EOF
#!/bin/bash
# PhaseZero managed: Big Picture Plus fallback when OpenGamepadUI is absent,
# plus gamescope-session-plus output resolution (boot-time detection).
# Regenerated by install-steamos-boot.sh; re-run after docking changes.

if ! command -v opengamepadui >/dev/null 2>&1; then
    CLIENTCMD="steam -gamepadui -steamos3"
fi

# Output resolution + preferred connector (resolved at install/dock time).
export SCREEN_WIDTH=$width_q
export SCREEN_HEIGHT=$height_q
export OUTPUT_CONNECTOR=$connector_q
EOF
}
...
remove_boot() {
    ...
    rm -f "$SERVICE_FILE" "$GRUB_SCRIPT" "$SESSION_FILE" "$HELPER_TARGET" \
        "$SESSION_TARGET" "$GRUB_HANDHELD_DROPIN" "$RESUME_SLEEP_UNIT"
    ...
    local home user_file
    home="$(target_home || true)"
    if [ -n "$home" ]; then
        for user_file in \
            "$home/.config/gamescope-session-plus/sessions.d/steam-plus" \
            "$home/.local/share/applications/phasezero-reboot-steamos-plus.desktop"; do
            if [ -f "$user_file" ] && grep -q 'PhaseZero managed' "$user_file" 2>/dev/null; then
                rm -f -- "$user_file"
            fi
        done
    fi
    ...
    grub_timeout="$(
        (
            ...
            for dropin in /etc/default/grub.d/*.cfg; do
                # GRUB drop-ins are discovered at runtime.
